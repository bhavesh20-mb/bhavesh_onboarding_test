...
import { Result, sleep } from 'src/utils/util';
...
export default async function () {
    if (LOCK_FARM_LIST_MAINNET.length > 0) {
        for (let iCnt = 0; iCnt < LOCK_FARM_LIST_MAINNET.length; iCnt++) {
            const currentFarm = LOCK_FARM_LIST_MAINNET[iCnt];
            try {
                ...
                while (true) {
                    const eventHistories = await fetch(
                        `https://api.ftmscan.com/api?module=logs&action=getLogs&address=${currentFarm.address}&fromblock=${startBlockNumber}&toblock=99999999&sort=asc&apikey=${ftmscanApiKey}`,
                    ).then((res: any) => res.json());

                    if (eventHistories.message === 'OK') {
                        const stakingHistories = eventHistories.result.filter(
                            (history: EventHistory) => history.topics[0] === StakeFuncHex,
                        );
