...
import { ftmscanApiKey, VOTING_TESTNET } from 'src/constants';
...
export default async function () {
    try {
        ...
        const voteDelayTimeData = await getVoteDelayingTime(FANTOM_TESTNET, VOTING_TESTNET);
        ...
        await VoteFnftModel.deleteMany({
            ...
            contract: VOTING_TESTNET.address.toLowerCase(),
        });

        const originalInfo = await VoteFnftModel.find({ contract: VOTING_TESTNET.address.toLowerCase() }).select(
            '-_id -__v',
        );

        const startBlockApiResult = await fetch(
            `https://testnet.ftmscan.com/api?module=block&action=getblocknobytime&timestamp=${startTimeStamp}&closest=before&apikey=${ftmscanApiKey}`,
        ...
        let txHistories = await fetch(
            `https://testnet.ftmscan.com/api?module=account&action=txlist&address=${VOTING_TESTNET.address}&startblock=${startBlock}&endblock=99999999&sort=asc&apikey=${ftmscanApiKey}`,
        ).then((res: any) => res.json());
