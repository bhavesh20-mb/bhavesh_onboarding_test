...
import { BondingFarmType, BONDING_FARMS } from 'src/constants/bondingFarm';
...
export default async function () {
    try {
        ...
        const bondingFarms = docs.filter((item) => item.contract.chainId === FANTOM_MAINNET.id);

        if (bondingFarms.length > 0) {
            for (let i = 0; i < bondingFarms.length; i++) {
                const bondingFarm = BONDING_FARMS.filter(
                ...
                try {
                    while (true) {
                        const eventHistories = await fetch(
                            `https://api.ftmscan.com/api?module=logs&action=getLogs&address=${bondingFarms[i].contract.address}&fromblock=${startBlockNumber}&toblock=99999999&sort=asc&apikey=${ftmscanApiKey}`,
                    ...
                    console.log('mainnet bonding farm fetch error =>', error);
                }
