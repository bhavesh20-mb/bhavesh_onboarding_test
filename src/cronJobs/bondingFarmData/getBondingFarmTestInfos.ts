...
export default async function () {
    try {
        ...
        const bondingInactiveFarms = docsInactive.filter((item) => item.contract.chainId === FANTOM_TESTNET.id);
        ...
        if (bondingFarms.length > 0) {
            ...
            for (let i = 0; i < bondingFarms.length; i++) {
                ...
                const bondingFarmName = await getBondingFarmName(FANTOM_TESTNET, currentBondingFarm.contract.address);
                await sleep(100);

                const autoStaking = await getAutoStakingStatus(FANTOM_TESTNET, currentBondingFarm.contract.address);
                await sleep(100);
