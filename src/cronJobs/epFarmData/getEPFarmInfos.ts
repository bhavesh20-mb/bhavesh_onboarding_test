...
export default async function () {
    if (LOCK_FARM_LIST_MAINNET.length > 0) {
        ...
        for (let i = 0; i < LOCK_FARM_LIST_MAINNET.length; i++) {
            const lockFARM = LOCK_FARM_LIST_MAINNET[i]!;
            ...
            if (!lockFARM.tokensForLp) {
                ...
                const totalToken0Amount: FtmScanInfo = await fetch(
                    `https://api.ftmscan.com/api?module=account&action=tokenbalance&contractaddress=${token0Address}&apikey=${ftmscanApiKey}&tag=latest&address=${lockFARM.stake.address}`,
                ).then((res) => res.json());
                const totalToken1Amount: FtmScanInfo = await fetch(
                    `https://api.ftmscan.com/api?module=account&action=tokenbalance&contractaddress=${token1Address}&apikey=${ftmscanApiKey}&tag=latest&address=${lockFARM.stake.address}`,
                ).then((res) => res.json());
